...
public class TypeDAOImpl extends AbstractTypeDAO {
	private static final Logger log = LoggerFactory.getLogger(TypeDAOImpl.class);
	// property constants
	...
	public Type findById(java.io.Serializable id) {
		log.debug("getting Type instance with id: " + id);
		try {
			Type instance = (Type) getHibernateTemplate().get(Type.class,
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public static TypeDAOImpl getFromApplicationContext(ApplicationContext ctx) {
		return (TypeDAOImpl) ctx.getBean("TypeDAO");
	}
}